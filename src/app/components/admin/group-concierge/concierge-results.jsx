...
class ConciergeResults extends Component {
	constructor(props) {
		super(props);
		this.module = 'concierge_results';
		this.state = {
			dataLoaded: false,
			loading: false,
			data: [],
	...
	getHeaderKeys = () => {
		return [
      {
			...
			},
			{
				...
				render: (_text, record) => (
					<span>
            <Tooltip title="View/Edit Patient">
              <Button
              ...
              >
                <EditOutlined />
						...
						<Tooltip title="Download/Print Report">
							<Button
								onClick={() => window.open("/group-patient-report/" + window.btoa(record.id) + '/' + record.confirmation_code)}
							>
	...
	handleTableChange = async (pagination, filters, sorter, manual) => {
		...
		this.setState({ loading: true });
		this.props.groupConciergeResults({
			filters: filters,
	...
	exportData = async (format) => {
		...
		await ReportsAPI.exportGroupData(format, {
			filters: allFilters
		})
			.then(response => {
	...
	render() {
		...
		const eventsFilter = () => {
			return (this.state.events_list.length > 0) ?
				<Select
					showSearch
					mode="multiple"
