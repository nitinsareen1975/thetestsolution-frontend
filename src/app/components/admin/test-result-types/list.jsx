...
class ManageTestResultTypes extends Component {
  constructor(props) {
    super(props);
    this.module = 'test_result_types';
    this.state = {
      dataLoaded: false,
      loading: false,
      data: [],
  ...
  getHeaderKeys = () => {
    return [
      ...
      },
      {
        title: "SNOMED",
        dataIndex: "snomed",
        filteredValue: this.getSelectedFilterValue('snomed'),
        ...this.getColumnSearchProps("snomed")
      },
      {
        title: "Status",
        dataIndex: "status",
        ...
        render: (_text, record) => (
          <span>
            <Tooltip title="Edit Result Type">
              <Button
                onClick={() => this.props.history.push("./test-result-types/edit/" + record.id)}
              >
                <EditOutlined />
              </Button>
            </Tooltip>
      ...
      <div className="gray-bg">
        <Row gutter={24}>
          ...
          <Col xs={12} sm={12} md={12} lg={12} xl={12}>
            <Button
              type="primary"
              onClick={() => this.props.history.push("./test-result-types/add")}
